...
left_prompt_segment() {
  ...
  [[ -n "$3" ]] && background="%K{default}" || background="%k"
  [[ -n "$4" ]] && foreground="%F{$3}" || foreground="%f"
  ...
  if [[ -n $6 ]]; then
    ...
    if [[ -n "$visual_identifier" ]]; then
      ...
      set_default $visual_identifier_color_variable "${backgroundColor}"
...
prompt_os_icon() {
  "$1_prompt_segment" "$0" "$2" "white" "black" "$OS_ICON"
}
